...
│   │   │   └── ticket_001/     # 草稿工单目录
│   │   │       ├── metadata.json # 工单配置 (声明涉及模块、指定 AI 编码引擎 [claude/codex/cursor] 等)
│   │   │       ├── module_prd.md # 纯业务 PRD (100% 纯业务，不涉及技术，不包含任何代码)
│   │   │       ├── spec.md       # 技术规范与契约 (遵循规格驱动开发，API 契约 + UI 交互规范)
│   │   │       ├── preview.html  # 仅视觉需求单需要；经人工审查的局部 HTML 流程与关键交互
...
3. **技术规格书编制 (Spec 定义)**：
  ...
  - **技术硬约束定义**：由工程师（或辅助架构 Subagent）在 `spec.md` 中统一定义本工单涉及的多端接口格式、API 数据 Schema、技术架构细节、契约规范、UI 交互约定与公共状态机。
4. **视觉预览评审（按需）**：
  - **适用条件**：仅适用于新增或实质性修改可见界面、主要任务流程或交互状态的需求单。
  - **行为**：在 `spec.md` 后、任务拆解前创建单文件 `preview.html`。它基于 `.codoop-flow/ui-snapshots/` 中经核对的基线还原已有页面，保留产品导航和视觉上下文，再用模拟数据和关键交互展示工单改动。仅没有现有界面时独立设计；这是设计预览，不是实际实现。
  - **发布门禁**：将 `metadata.json.visual_preview` 设为 `true`；发布时必须具备已审查的 `preview.html`。运行时截图仍独立由 `ui_capture` 控制。
5. **原子任务分解设计 (Plan & Todo 编制)**：
...
6. **定稿归档与工单发布 (Promote to Pending)**：
  - **行为**：当 `module_prd.md`、`spec.md`、`plan.md`、`todo.md`、`metadata.json`，以及 `visual_preview` 为 true 时的 `preview.html` 全部编制完成并通过人工审核确认（或协作 Agent 状态置为 `FINALIZED`）后，通过本地 light-weight CLI 工具、Skill 自动化脚本或人工直接**将该工单文件夹整体从 `docs/tickets/drafts/` 剪切/移动到 `docs/tickets/pending/` 下**。
  - **唤醒**：工单进入 `pending/` 区即视为正式定稿发布，下一次 Mac `launchd` 或定时调度任务唤醒时，`main_scheduler.py` 将自动提取该工单进入开发执行环。
7. **验收上线与常青沉淀 (Launch & ADR)**：
...
1. **多引擎/插件式渐进编码阶段 (Build)**：
  ...
  - **极致渐进披露与技术标准对齐**：无论采用何种编码引擎，调度器启动时会披露该工单目录下的 `module_prd.md`、`spec.md`，以及存在时的 `preview.html`，再动态披露 `docs/tech/project-structure.md` 和 `docs/tech/tech-standards.md`，作为其必须遵守的技术边界与架构标准约束。Scope 只是改动指引：既避免范围失控，也允许 required gate 所需的最小相邻根因修复。
2. **自动化沙盒校验与视觉捕获阶段 (Verify)**：
  - **规范遵循**：遵循 `test-driven-development`。
  - **独立且识别基线的步骤**：改动前将 lint、build、focused test 和需要时的 UI capture 分别运行并记录，绝不使用 `lint && build && test`。某一步失败不得抑制之后独立的证据采集。每条诊断标准化为 module、step/command、文件、行号（如有）、rule/错误码和标准化文本；改动后对照改动前基线和 diff 文件列表。只有新增/变化诊断或 diff 文件中的诊断才是当前工单失败；完全未变且不在 diff 中的诊断是需要完整报告的 baseline blocker，不触发自愈，也不是宽泛的失败豁免。
  - **自适应 UI/UX 视觉捕获**：当工单修改涉及到前端、用户界面或 UI 交互时，捕获流程必须自动拉起对应的渲染工具：
...
3. **系统级纠错分端自愈阶段 (Debug)**：
  ...
  - **即时反馈自愈**：新增/变化诊断、diff 文件中的诊断、功能门禁失败、截图异常或评审拒绝才触发 **Stop-the-line (停止整线)** 熔断机制。完全匹配的 baseline blocker 不消耗次数，也绝不触发自愈。
    ...
    - **最大尝试限制**：自愈请求限制最大 3 次。若当前工单失败重试耗尽仍未通过，终止运行，进行熔断保护并移动至 `failed/` 目录，输出包含所有报错细节的 `healing_report.md`。若功能门禁与评审均通过而只剩 baseline blocker，则在 finish 交接中保留精确的 `baseline_warnings`。目标配置要求全仓绿灯时，使用 `blocked_by_baseline`，保留 worktree 与证据，绝不标记为 `failed`。
...
| **2. 需求设计定义 (Define)**       | `spec-driven-development` `api-and-interface-design`                                      | **人类提单人主导 + Product Agent 协同** (读取 `source/agency-agents-main/product/product-sprint-prioritizer.md` 中的产品总监角色)                                                                                                                                                                                                                | 1. **纯业务 PRD (module_prd.md)**：人类协同 PM 编写纯粹的业务描述说明书，100% 纯业务属性，绝不涉及代码或数据库细节。 2. **规格驱动 Spec (spec.md)**：遵循 `spec-driven-development` 原则，针对业务 PRD 编制技术规格规范。3. **按需预览 (preview.html)**：视觉需求单在规划前由人工评审局部静态 HTML 预览；`visual_preview` 为 true 时必须存在。 |
...
| **4. 隔离渐进开发 (Build)**        | `incremental-implementation` `context-engineering`                                        | **抽象 AI 编码引擎** (仅支持 Claude Code CLI, Codex CLI/API, Cursor CLI)                                                                                                                                                                                                                                                               | 1. **渐进披露**：调度器启动编码引擎时向其披露该工单局部工作树内的 `module_prd.md`、`spec.md`，以及存在时的 `preview.html`，杜绝上下文膨胀与大范围修改。 2. **范围纪律**：将 Scope 作为改动指引；required gate 需要时可做最小相邻根因修复，并在报告中说明。 |
| **5. 自动化校验与视觉捕获 (Verify)**   | `test-driven-development`                                                                 | **自适应多端视觉捕获代理工具**                                                                                                                                                                                                                                                               | **自适应 UI/UX 视觉捕获**：系统根据工单所属子系统智能匹配捕获工具（Web 端拉起 Playwright，移动端拉起模拟器/Appium，PC 桌面端视架构而定拉起对应驱动）。截图不限制任何图像分辨率尺寸，无暗黑模式截图硬性限制，截图强制保存在局部隔离路径下，完全规避冲突。                                                                                                       |
