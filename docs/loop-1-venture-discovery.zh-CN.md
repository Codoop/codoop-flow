...
在分派角色或创建共享草稿前，Skill 会先加载 `grilling` 并进行简短的 Discovery Intake（冷启动引导）。它先查证可获得的项目事实，再一次只用大白话问一个需要用户决定的问题，依次确认产品想法、目标用户、核心流程和视觉方向。每题可给示例或选项，并始终允许回答“我不确定，请推荐”。平台、商业模式、数据风险和技术约束只在前面的回答表明相关时再问。

随后 Skill 会汇总一份 Discovery Brief：产品定位、用户与场景、第一版范围、视觉方向、已知约束和待决问题。你确认或修正后，才开始多角色协作。
...
**产品-敏捷-优先级化** 代理起草：
- `requirements.md` — 业务概览、用户故事、状态转移、Gherkin BDD 验收标准
...
**设计-UI-设计师** 负责产出：
- `design-system.md` — 唯一的视觉设计事实来源：视觉方向、色彩、字体、间距、组件语言、页面构图、动效和视觉无障碍
...
至此，Discovery Intake 已建立初始 Brief；其余输入会在需要时通过 SNAP 澄清问题逐步收集。
