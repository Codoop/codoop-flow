...
| # | 决策点 | 确认方案 | 实施方式 |
|---|--------|--------|--------|
| 1 | **Spec/Plan 提升策略** | ✅ 保持相同逻辑，通过上下文区分 | 独立调用和工单子步骤使用同一套 SKILL.md，通过传入的上下文（PRD 内容）自动适配 |
| 2 | **第一环产出物读取** | ✅ 自动扫描并注入 | codoop-ticket SKILL.md 自动读取 `docs/backlog/` 下的产品/设计/架构规范，作为 context 传给 PM/Architect agents |
| 3 | **子 skills 调用方式** | ✅ 清晰指引 + 文本上下文 | codoop-ticket SKILL.md 在调用 spec-driven-development 和 planning-and-task-breakdown 时，通过 prompt 传入前一阶段的产出（PRD、Spec）作为上下文 |
| 4 | **人类确认触发** | ✅ 明确确认点 | 每阶段完成后，SKILL 明确提示用户"请 review xxx.md，满意后说 OK 进入下一阶段" |
| 5 | **promote 前检查** | ✅ 复用 tickets_cli validate | 调用现有的 `tickets_cli validate` 检查必需字段，无需新增检查规则 |
