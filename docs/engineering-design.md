...
flowchart TD
    ...
    subgraph agentLoop [4. Agent-Centric Loop]
        ...
        A1 -->|"3. Local Test Sandbox"| A2["2. Adaptive Test Sandbox (Verify)<br>Execute the ticket's explicit test_command<br>Adaptive headless screenshots (no pixel-size limit)<br>docs/tickets/in_progress/[id]/public/qa-screenshots/<br>Skill: test-driven-development"]
...
2. **Automated Sandbox Verification & Visual Capture Stage (Verify)**:
  ...
  - **Adaptive Multi-Platform Testing & Toolchain Mapping**: Based on the platform subsystems declared in the current ticket's `metadata.json`, the scheduling system adaptively runs the specific automated tests:
    - Runs each modified module's explicitly configured `test_command` from `metadata.json`. Projects choose the command and its location; commands must provide sandbox isolation where needed to avoid dirty-data pollution.
