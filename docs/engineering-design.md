...
flowchart TD
    subgraph ventureLoop [1. Venture-Discovery Loop]
        V1["1. Top-Level Design & Business Justification<br>Skill: codoop-discover (in-session)<br><b>Subagents:</b><br>- product-sprint-prioritizer (PM)<br>- sales-offer-lead-gen-strategist (GTM Pricing)<br>- design-ui-designer (Visual UI)<br>- engineering-backend-architect & engineering-software-architect (Tech)<br>- alignment-agent (Consistency Auditor)"] -->|"2. Collaborative Drafting"| V2["docs/backlog/design-draft.md<br>(Challenge Loop Conflict Alignment)"]
    ...
    subgraph agentLoop [4. Agent-Centric Loop]
        ...
        A1 -->|"3. UI evidence"| A2["2. UI Screenshot Gate (Verify)<br>Adaptive headless screenshots (no pixel-size limit)<br>docs/tickets/in_progress/[id]/public/qa-screenshots/<br>Skill: test-driven-development"]
        A2 --> A3{"UI evidence captured?"}
...
- `**interface/` (Visual Design Layer)**:
  - `design-system.md`: The visual-design source of truth: visual direction, principles, tokens, component language, page composition, motion, and visual accessibility. It excludes mockups and implementation details.
...
- **UI Designer (Visual Design Expert)**: Uses `./source/agency-agents-main/design/design-ui-designer.md` to produce the visual design system and page-composition rules. Product user journeys remain owned by the PM.
...
│   │   ├── interface/          # Visual design layer (design-system.md)
...
│   │   │       ├── metadata.json # Ticket configuration (declares involved modules, designated AI coding engine [claude/codex/cursor], etc.)
...
│   │   │       ├── preview.html  # Required only for a visual feature ticket; reviewed local HTML flow and key interactions
...
├── deploy/                     # Business-oriented deployment and operations scripts
│   ├── README.md                # Business index, entry points, prerequisites, and usage guidance
│   └── <business-name>/         # User-defined business directories; no fixed platform naming
├── resources/                  # Cross-platform shared product resources
│   ├── README.md                # Resource index, consumers, source, and licence guidance
│   ├── audio/                  # Shared audio assets
│   └── <resource-group>/        # User-defined groups such as branding, icons, images, or fonts
...
6. **Finalize, Archive & Ticket Release (Promote to Pending)**:
  - **Behavior**: Once `module_prd.md`, `spec.md`, `plan.md`, `todo.md`, `metadata.json`, and `preview.html` when `visual_preview` is true are authored and confirmed through human review (or the collaborating Agent sets the status to `FINALIZED`), the entire ticket folder is **cut/moved from `docs/tickets/drafts/` to `docs/tickets/pending/`** via a local light-weight CLI tool, a Skill automation script, or manually.
...
7. **Acceptance, Launch & Evergreen Accumulation (Launch & ADR)**:
...
1. **Multi-Engine / Plugin-Style Progressive Coding Stage (Build)**:
  ...
  - **Extreme Progressive Disclosure & Technical-Standard Alignment**: Regardless of the coding engine used, at startup the scheduler discloses the ticket directory's `module_prd.md`, `spec.md`, and `preview.html` when present, then dynamically discloses `docs/tech/project-structure.md` and `docs/tech/tech-standards.md` as the technical boundaries and architecture standards it must comply with. Scope remains change guidance, allowing the smallest necessary adjacent root-cause repair without scope creep.
2. **Automated Sandbox Verification & Visual Capture Stage (Verify)**:
  - **Spec Compliance**: Follows `test-driven-development`.
  - **Independent Baseline-Aware Steps**: Before editing, run lint, build, focused test, and required UI capture as separately recorded steps, never `lint && build && test`. A failed step cannot suppress later independent evidence collection. Normalize each diagnostic into module, step/command, file, line (if present), rule/error code, and normalized text; compare post-change diagnostics with the pre-change baseline and the diff file list. Only a new/changed diagnostic or one in a diff file is a ticket failure. An exact unchanged diagnostic outside the diff is a reported baseline blocker, not a self-heal trigger or a failure exemption.
  - **Adaptive UI/UX Visual Capture**: When the ticket modification involves the front end, user interface, or UI interaction, the capture flow must use the corresponding rendering tools:
...
3. **System-Level Correction & Per-Platform Self-Healing Stage (Debug)**:
  ...
  - **Instant Feedback Self-Healing**: A ticket failure (new/changed diagnostics, a diff-file diagnostic, failed functional gate, abnormal screenshot collection, or review rejection) triggers the **Stop-the-line** circuit-breaker mechanism. Exact baseline blockers do not consume attempts and never trigger self-healing.
    ...
    - **Maximum Attempt Limit**: Self-healing requests are limited to a maximum of 3 times. If ticket failures still do not pass, the run is terminated, circuit-breaker protection is applied, the ticket is moved to the `failed/` directory, and a `healing_report.md` containing all error details is output. If all feature gates and review pass with only baseline blockers, preserve exact `baseline_warnings` in the finish handoff. A target configured for a whole-repository green run uses `blocked_by_baseline`, retaining worktree and evidence, never `failed`.
...
| ---------------------------- | ----------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **1. Top-Level Business & Architecture Justification (Discovery)** | `product-discovery-loop`                                                                  | **Product Discovery Subagents** (top-tier expert roles loaded by the main brain from `source/agency-agents-main/`: `product-sprint-prioritizer` (product), `sales-offer-lead-gen-strategist` (GTM and pricing), `design-ui-designer` (visual design), `engineering-backend-architect` & `engineering-software-architect` (technical architecture), `workflow-architect` (consistency hard-lock audit)) | 1. **Full-Chain Justification**: The PM, business-strategy expert, UI designer, and chief technical architect conduct 0-to-1 de-assumption top-level justification, polishing the 5 dimensions of specification drafts in `docs/backlog/`. 2. **Audit Lock**: After `workflow-architect` performs a 100% consistency audit of the drafts, they are solidified, merged into the evergreen docs, and the backlog Staging area is cleared.                                                                                                                   |
| **2. Requirement Design Definition (Define)**       | `spec-driven-development` `api-and-interface-design`                                      | **Human Submitter-Led + Product Agent Collaboration** (reads the product-director role in `source/agency-agents-main/product/product-sprint-prioritizer.md`)                                                                                                                                                                                                                | 1. **Pure-Business PRD (module_prd.md)**: The human, collaborating with the PM, writes a purely business description document—100% pure-business attribute, absolutely no code or database details. 2. **Spec-Driven Spec (spec.md)**: Following the `spec-driven-development` principles, authors the technical specification targeting the business PRD. 3. **Conditional Preview (preview.html)**: For a visual feature, the human reviews a ticket-local static HTML preview before planning; it is required when `visual_preview` is true. |
...
| **4. Isolated Progressive Development (Build)**        | `incremental-implementation` `context-engineering`                                        | **Abstract AI Coding Engine** (supports only Claude Code CLI, Codex CLI/API, Cursor CLI)                                                                                                                                                                                                                                                               | 1. **Progressive Disclosure**: When the scheduler starts the coding engine, it discloses `module_prd.md`, `spec.md`, and `preview.html` when present within that ticket's local work tree, preventing context bloat and large-scope modifications. 2. **Scope Discipline**: Treat Scope as change guidance; make and report the smallest necessary adjacent root-cause repair when a required gate needs it. |
| **5. Automated Verification & Visual Capture (Verify)**   | `test-driven-development`                                                                 | **Adaptive Multi-Platform Visual Capture Agent Tools**                                                                                                                                                                                                                               | **Adaptive UI/UX Visual Capture**: The system intelligently matches capture tools according to the ticket's subsystem (Web spins up Playwright, Mobile spins up a simulator/Appium, PC desktop spins up the corresponding driver depending on the architecture). Screenshots have no restriction on image resolution size, no hard dark-mode-screenshot requirement, and are forcibly saved in the local isolated path, completely avoiding conflicts.                                                                                                       |
