...
`codoop-ticket` 继续向你确认产品规则、范围、优先级和验收体验，但问题应围绕你能直接判断的结果；每次最多提出 1–3 个高价值问题，并在有建议时说明推荐项。

例如，不问“筛选条件使用 AND 还是 OR？”，而问：“用户同时选了‘跑鞋’和‘300 元以下’时，应该只显示同时满足两个条件的商品，还是满足任一条件也显示？我建议前者，结果会更准确。”

API 形式、数据库索引、框架模式、状态管理、测试布局等常规技术选择，由 agent 先结合现有项目约定自行决定，并记录在 `spec.md`；除非它会实质改变用户体验、交付范围、费用、隐私/合规、不可逆数据行为或全局架构契约，才向你解释其用户影响后请求确认。你的自然语言回答会被 agent 转写为正式的用户故事、验收标准和技术约束。
...
4. **审查** — agent 先说明用户能获得什么、刻意不做什么、还需要你决定什么；你再审查并确认或请求更改
...
阶段 3 后，`codoop-ticket` 调用 `update_metadata_from_docs` 从你的 `spec.md` 和任务文件自动推断 `metadata.json`。它还会检查已确认的规格是否改变了用户看得到、点得到的界面、交互或任务流程。若有改变，它会用大白话询问是否要在交付前检查实际画面和交互、保存截图并由 UI/UX reviewer 查看，并建议开启；纯后端、基础设施、重构或内部功能则保持关闭，不增加无意义的确认。人工审查结果并在验证前确认或修改。
