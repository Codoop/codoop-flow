...
**你只需用自然语言指挥 Codex 或 Claude，脏活累活由脚本护栏兜底。** 智力活（写码、自愈、评审判断）交给当前 agent 会话；体力活（挑单、管 git worktree、检查必需 UI 截图证据、提交）交给一个不会幻觉的确定性 CLI。它是可迁移工具，本身不含业务代码——用一份 `codoop_flow.toml` 指向你真正要开发的工程即可。
...
请安装完整插件；其中的 Skill 共用一份 Runtime，不再单独作为 Claude 插件发布。
...
3. **验证** — 独立运行 lint/build/focused test；UI 截图硬门（如需）
...
- **基线感知验证**：完整报告既有诊断；只有新增、变化或 diff 文件中的诊断才会触发自愈或失败
- **确定性验证**：必需的 UI 截图门禁无法绕过
...
```text
使用 codoop-init skill，分析这个已有仓库并初始化 codoop-flow。
```
...
新项目固定使用 `backend/`、`web/`、`desktop/`、`mobile/`。`codoop-init`
只创建用户选择的目录，目录内只有 `.gitkeep`，不会生成可运行项目。
...
2. **脚本护栏**（`codoop_tools.py`）：必须 100% 确定、不能幻觉的活——
   ...
   - 验证：独立运行 lint/build/focused test、检查 UI 截图
   - 提交和归档（in_progress → done）
   - 优雅处理失败

3. **评审 persona**（在 `runtime/codoop-flow/agents/` 中）：验证通过后，agent 运行多个评审者——
...
| **脚本** | 保证隔离、UI 截图门禁、无法绕过 | worktree 生命周期、验证门禁 |
