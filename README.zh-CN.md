...
### 📋 第二环：人工设计（工单设计）
通过三个阶段精心设计工单：需求文档（PRD）→ 技术规格（Spec）→ 任务分解（Plan）。准备好交给第三环。澄清时先讨论用户能直接判断的目标和体验；技术实现由 agent 写入专业、明确的 Spec，只有会影响体验、成本、安全或范围时才需要你确认。
...
# 编辑 drafts/ticket_001/ 里的 module_prd.md（业务）、spec.md（契约）
