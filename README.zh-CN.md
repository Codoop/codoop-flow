...
3. **验证** — 硬门：测试全过、UI 截图（如需）。编辑范围仅作参考，不强制
...
| 特性 | 好处 |
|------|------|
| **确定性验证** | 硬门（测试 + UI）无法被 AI 幻觉绕过；编辑范围仅作参考 |
...
# 编辑 drafts/ticket_001/ 里的 module_prd.md（业务）、spec.md（契约 + files_to_edit 范围提示）
...
| `files_to_edit` | **编辑范围提示**（glob 模式）。引导 agent 该在哪里改动；`verify` 不强制 |
...
必填：`ticket_id / title / modules / test_command`。`ticket_type`（默认 `feature`）与 `files_to_edit` 可选。
...
| `verify <id>` | 在 worktree 跑测试 + (UI) 截图硬门禁。编辑范围仅作参考，不强制 |
